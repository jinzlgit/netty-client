...
(function ($) {
    $.extend({
    	...
    	modal: {
            ...
            // 弹出层指定宽度
            open: function (title, url, width, height, callback, data) { //data是为异常事件页面-批量处理功能额外添加的参数
            	//如果是移动端，就使用自适应大小弹窗
			...
			openVehicle: function (title, url, width, height, callback) {
				//如果是移动端，就使用自适应大小弹窗
				if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
					width = 'auto';
					height = 'auto';
        ...
        operate: {
			...
			editStatus: function(id,status) {
				$.modal.confirm("确定修改该条" + $.table._option.modalName + "状态吗？", function() {
					var url = $.common.isEmpty(id) ? $.table._option.editStatus : $.table._option.editStatus.replace("{id}", id);
					if($.table._option.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
			...
			//修改执行状态
			editExecuteStatus: function(status,id) {
        		var url =  $.table._option.editExecuteStatus;
        		var data = { "id": id,"status":status};
        		$.operate.submit(url, "post", "json", data);
			},
            // 批量删除信息
            ...
            dealwith: function() {
				...
				var url = $.table._option.dealwithUrl;
				return rows;
            },
            // 清空信息
            clean: function() {
			...
			addGroups: function(id) {
				$.modal.openFull("添加编组", $.operate.addUrl(id));
			},
			//添加扩展
			addExtend: function(id){
				$.modal.open("添加" + $.table._option.modalName, $.operate.addExtendUrl(id));
			},
			//列车管理->新增传感器
			addSensor: function(id){
				$.modal.open("新增传感器", $.operate.addSensorUrl(id));
			},
			//批量修改调度信息
			batchModifyDispatch: function(){
				var rows = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
				if (rows.length == 0) {
					$.modal.alertWarning("请至少选择一条记录");
					return;
				...
				var data = { "ids": rows.join() };
				url = $.table._option.batchModifyDispatchUrl.replace("{id}", data.ids);
				$.modal.open("批量修改",url);
			},
            // 添加信息，以tab页展现
            addTab: function (id) {
			...
			//添加扩展访问地址
			addExtendUrl: function(id) {
				var url = $.common.isEmpty(id) ? $.table._option.extendUrl : $.table._option.extendUrl.replace("{id}", id);
				return url;
			},
			addSensorUrl: function(id) {
				var url = $.common.isEmpty(id) ? $.table._option.addSensorUrl : $.table._option.addSensorUrl.replace("{id}", id);
				return url;
			},
            // 修改信息
            edit: function(id) {
			...
			// 修改车次信息
			updateTrainNumberInfo: function() {
        		var url = $.table._option.updateTrainNumberUrl;
        		$.modal.open("修改车次信息", url);
			},
            // 修改信息，以tab页展现
            editTab: function(id) {
